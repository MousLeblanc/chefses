...
// 1. Définir la fonction de callback pour gérer les nouveaux boutons
function handleNewButtons(addedNodes) {
  addedNodes.forEach(node => {
    // Vérifier les boutons directs
...
// 2. Initialisation d'un bouton (exemple)
function initButton(button) {
  if (button.dataset.initialized) return; // Éviter les doublons
  ...
  button.dataset.initialized = 'true'; // Marquer comme initialisé
}

// 3. Configuration du MutationObserver
const observer = new MutationObserver((mutations) => {
  for (const mutation of mutations) {
    if (mutation.addedNodes.length) {
      handleNewButtons(Array.from(mutation.addedNodes));
    }
  }
});
...
observer.observe(document.body, { 
  childList: true, 
  subtree: true,
  attributeFilter: ['class'] // Optionnel : surveiller les changements de classe
});

// 5. Nettoyage (si nécessaire)
// window.addEventListener('unload', () => observer.disconnect());    // Ré-initialiser les boutons seulement si nécessaire
