...
class SupplierManager {
    constructor() {
        ...
        if (this.suppliers.length > 0 && this.selectedSupplierId === null) {
  this.selectSupplier(this.suppliers[0].id);
}
...
function openOrderModal(supplierId) {
  ...
  if (!modal) {
    console.warn("Modal 'order-modal' introuvable.");
    return;
  }

  

  // Exemples de contenu injecté dynamiquement
  const body = document.getElementById('order-modal-body');
  body.innerHTML = `
    <p>Commande pour le fournisseur : ${supplierId}</p>
    <form id="order-form">
  ...
  document.getElementById('order-form').addEventListener('submit', function(e) {
    ...
    console.log(`Commande : ${quantity} de ${ingredient} pour fournisseur ${supplierId}`);
